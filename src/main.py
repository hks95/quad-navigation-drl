...
def play_game(train_indicator = 1, debug=True):
    ...
    for e in range (eps_count):
        ...
        while(done == False):
            ...
            noise_t[0][0] = train_indicator * max(epsilon, 0) * ou_func(a_t_original[0][0],  0.0 , 0.60, 0.30)
            noise_t[0][1] = train_indicator * max(epsilon, 0) * ou_func(a_t_original[0][1],  0.0 , 0.60, 0.30)
            noise_t[0][2] = train_indicator * max(epsilon, 0) * ou_func(a_t_original[0][2],  0.0 , 0.60, 0.30)
    ...
    if plot_reward:
        ...
        # fig2.show()
        save_path = './plots/training_reward.png'
        plt.savefig(save_path)
